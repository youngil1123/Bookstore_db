...
$sql =" INSERT INTO pay VALUES(NULL, 0, '".$sum."', '".$sum*0.1."', '".$sum*0.9."','".$plan."', '".$no."');";
echo "Query : ".$sql;
...
if($ret) {
    ...
    echo $sql;
    $sql =" INSERT INTO delivery VALUES('".$d_number."', now(), '".$name."', '".$contact."', '".$address."');";
    $ret6 = mysqli_query($con, $sql);

    echo $row['name'];

    echo "<html><script>alert('결제가 성공적으로 완료되었습니다.');
        location.replace('nonemembermain.html')</script></html>";  // 임시로 로그인 화면으로 연결
