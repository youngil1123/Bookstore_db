...
$foreign_no=$_POST["number"]; if(empty($foreign_no)){ $foreign_no='%';}else{$foreign_no='%'.$foreign_no.'%';};
...
$sql ="SELECT * from foreignbook where foreign_no like '{$foreign_no}' 
and `name` like '{$name}'
...
echo "<TABLE border=1>";
echo "<TR>";
echo "<TH>외국도서번호</TH><TH>외국도서이름</TH><TH>판매가</TH><TH>저자</TH><TH>출판사</TH>";
echo "<TH>쪽수</TH><TH>출시일</TH><TH>상태</TH>";
echo "</TR>";
while($row = mysqli_fetch_array($ret)) {
    echo "<TR>";
    echo "<TD>", $row['foreign_no'], "</TD>";
    echo "<TD>", $row['name'], "</TD>";
    ...
    <script>
        function btn_click(str) {
            if (str == "add") {
                ...
                admin.action = "deleteforeignbook.php";
            }
        }
    </script>
</head>
...
<FORM Name="admin" METHOD="POST">
    <input type="hidden" name="tbl" value="foreignbook">

    외국도서번호 : <input type="text" name="number"><br>
    외국도서이름 : <input type="text" name="name"><br>
