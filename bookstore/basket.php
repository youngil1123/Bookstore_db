...
if($gubun == "sangpum") $sql = "insert into basket set basket_no=NULL, amount=1, sum='$sinfo[price]', ID='$id', gift_number='$gift_no'";
else if($gubun == "music") $sql = "insert into basket set basket_no=NULL, amount=1, sum='$sinfo[price]', ID='$id', record_number='$record_no'";
else if($gubun == "ibook") $sql = "insert into basket set basket_no=NULL, amount=1, sum='$sinfo[price]', ID='$id', domestic_number='$domestic_no'";
else if($gubun == "obook") $sql = "insert into basket set basket_no=NULL, amount=1, sum='$sinfo[price]', ID='$id', foreign_number='$foreign_no'";
else if($gubun == "mungu") $sql = "insert into basket set basket_no=NULL, amount=1, sum='$sinfo[price]', ID='$id', stationery_number='$stationery_no'";
...
$sql2="SELECT SUM(`sum`) from basket group by id having id = '{$id}';";
...
<body>
    ...
    <table>
        <tr>
            ...
            <td>
                <form name="clear" method="post" action='basket_clear.php'>
                    <input type="hidden" name="id" value=<?php echo $id;?>>
                    <input type="submit" value="초기화">
                </form>
            </td>
            <td>
                <form name="account" method="post" action='pay_progress.php'>
